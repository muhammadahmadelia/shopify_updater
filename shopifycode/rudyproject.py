...
class Rudyproject_Shopify:
    def __init__(self, DEBUG: bool, config_file: str, query_processor: Query_Processor, logs_filename: str) -> None:
        ...
        self.template_file_path = 'templates/Rudyproject/'
    ...
    def controller(self, brands: list[Brand]) -> None:
        try:
            ...
            if self.DEBUG: print(f'Exception in Digitalhub_shopify controller: {e}')
            self.print_logs(f'Exception in Digitalhub_shopify controller: {e}')
    ...
    def create_product_title(self, brand: Brand, product: Product) -> str:
        ...
        try:
            ...
            if title_template:
                ...
                if str(product.name).strip(): title += f'{str(product.name).strip().title()} '
        ...
        try:
            ...
            for index, image_360_url in enumerate(product.metafields.img_360_urls):
                ...
                if product.lens_code:
                    ...
                    image_filename = f'{number}_{name}_{frame_code}_{lens_code}_{index+1}.jpg'
                else:
                    image_filename = f'{number}_{name}_{frame_code}_{index+1}.jpg'
                # download image
    ...
    def check_product_metafields(self, new_product_title: str, brand: Brand, product: Product, shopify_metafields: dict, shopify_processor: Shopify_Processor) -> None:
        try:
            ...
            if str(product.frame_color).strip():
                ...
                if metafield_found_status and not str(shopify_metafield__frame_color).strip():
            ...
            if str(product.lens_color).strip():
                ...
                if metafield_found_status and not str(shopify_metafield__lens_color).strip():
            ...
            if str(product.metafields.product_size).strip():
                ...
                if metafield_found_status and not str(shopify_metafield__product_size).strip():
