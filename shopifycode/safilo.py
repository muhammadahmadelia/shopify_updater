...
class Safilo_Shopify:
    def __init__(self, DEBUG: bool, config_file: str, query_processor: Query_Processor, logs_filename: str) -> None:
        ...
        self.template_file_path = 'templates/Safilo/'
    ...
    def controller(self, brands: list[Brand]) -> None:
        try:
            ...
            self.print_logs(f'Exception in Safilo_Shopify controller: {e}')
    ...
    def create_product_title(self, brand: Brand, product: Product) -> str:
        ...
        try:
            ...
            if title_template:
                ...
                if str(product.lens_code).strip(): title += f' {str(product.lens_code).strip().upper()}'
                # if str(brand.name).strip(): title += f'{str(brand.name).strip().title()} '
    ...
    def get_product_description_from_template(self, description_template: str, product: Product) -> str:
        product_description = ''
        try:
            product_description = str(description_template).strip().replace('{product.number}', str(product.name).strip().upper()).strip()
    ...
    def get_matched_variant(self, database_variant: Variant, shopify_variants: list[dict]) -> int:
        ...
        try:
            for index, shopify_variant in enumerate(shopify_variants):
                if str(shopify_variant['sku']).strip() == str(database_variant.sku).strip():
                    matched_index = index
                    break
        except Exception as e:
